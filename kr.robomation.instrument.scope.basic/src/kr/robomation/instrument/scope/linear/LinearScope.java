 ...
 */
public class LinearScope extends SystemComponent
{
	private Robot robot;
	...
	public void execute()
	{
		...
		if(selectedDevice != null)
			dataSequence.add(selectedDevice.read(indexOfData));
		dashboard.update();
	}
	...
	class ControlPanel extends AbstractDashboard
	{
		...
		protected void createContents(Shell shell)
		{
			...
			shell.setText("Linear Scope | " + robot.getName());
...
//			private Color beam = SWTResourceManager.getColor(255, 255, 0);
			private Color beamColor = ResourceManager.getColor(255,255,255); //64, 64, 64);
			private Color backColor = ResourceManager.getColor(64,64,64); //240, 255, 240);
			...
			public Panel(final ControlPanel parent, final Shell shell)
			{
				...
				canvas.addPaintListener(new PaintListener()
				{
					...
					{
						GC gc = e.gc;
						Image buffer = new Image(shell.getDisplay(), image, SWT.IMAGE_COPY);
			...
			private int buildWaveForm()
			{
				...
				if(selectedDevice != null)
				{
					for(int i = 0; i < WIDTH; i++)
					{
						...
						int norm = selectedDevice.getMax() - selectedDevice.getMin();
