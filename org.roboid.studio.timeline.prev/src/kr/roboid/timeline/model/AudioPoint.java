...
class AudioPoint extends ControlPoint
{
	...
	{
		org.roboid.studio.timeline.AudioPoint newPoint;
		if(parent instanceof org.roboid.studio.timeline.VoiceTrack)
			newPoint = TimelineFactory.eINSTANCE.createVoicePoint();
		else
			newPoint = TimelineFactory.eINSTANCE.createAudioPoint();
		copyTo(newPoint);
		return newPoint;
	}
	...
	void copyTo(org.roboid.studio.timeline.ControlPoint newPoint)
	{
		...
		((org.roboid.studio.timeline.AudioPoint)newPoint).setText(getFileName());
		((org.roboid.studio.timeline.AudioPoint)newPoint).setVolume(getVolume());
		if(newPoint instanceof org.roboid.studio.timeline.VoicePoint)
			((org.roboid.studio.timeline.VoicePoint)newPoint).setLipGain(getLipGain());
	}
}
